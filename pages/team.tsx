...
const Team: React.FC<TeamProps> = ({ isDemo }) => {
    ...
    useEffect(() => {
        const init = async () => {
            if (isDemo) {
                ...
                setLoading(false);
            } else if (viewingInstitutionId) {
                fetchTeam(viewingInstitutionId);
                setLoading(false);
            } else {
    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        try {
            const targetInstitutionId = viewingInstitutionId || currentUser?.institution_id;
            if (!targetInstitutionId) throw new Error("Instituição não identificada.");
            ...
            if (existingUser) {
                if (existingUser.institution_id === targetInstitutionId) {
                    alert("Este usuário já faz parte desta equipe.");
                    setIsInviting(false);
                    return;
                }

                if (window.confirm(`O usuário '${existingUser.full_name}' já possui cadastro no sistema.\nDeseja adicioná-lo diretamente à equipe agora?`)) {
                    // Adicionar diretamente (Transferir/Vincular)
                    const { error: updateError } = await supabase
                        .from('profiles')
                        .update({ institution_id: targetInstitutionId, role: 'staff' })
                        .eq('id', existingUser.id);

                    if (updateError) throw updateError;

                    alert("Usuário adicionado à equipe com sucesso!");
                    fetchTeam(targetInstitutionId);
                    setInviteEmail('');
            ...
            // 2. Se não existe, criar convite normal
    ...
    const handleDeleteMember = async (memberId: string) => {
        ...
        try {
            ...
            // 1. Buscar posts do usuário para apagar comentários neles
            const { data: userPosts } = await supabase.from('community_posts').select('id').eq('author_id', memberId);
            ...
            await supabase.from('community_comments').delete().eq('author_id', memberId);
            const { error: postsError } = await supabase.from('community_posts').delete().eq('author_id', memberId);
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            {(isLocalAdmin || !viewingInstitutionId) && <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-8">
                        ...
                        {loading ? (
                            ...
                            <tr key={member.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right">
                                    {(isLocalAdmin || (!viewingInstitutionId && member.role !== 'admin')) && (
